...
def create_document():
    ...
    # -------------------------------------------------------------
    h1 = doc.add_heading("1. Multi-Instance Architecture & IP Mapping", level=1)
    ...
    for row_idx, rdata in enumerate(rows_data):
        ...
        for col_idx, text in enumerate(rdata):
            ...
            if row_idx % 2 == 1:
                set_cell_background(row_cells[col_idx], "F7F9FB")

    doc.add_paragraph().paragraph_format.space_after = Pt(12)

    # -------------------------------------------------------------
    # Helper to add section with screenshot placeholder / image
    # -------------------------------------------------------------
    screenshots_dir = os.path.join(os.path.dirname(__file__), "screenshots")
    os.makedirs(screenshots_dir, exist_ok=True)

    def add_task_section(title, task_num, description, image_filename, expected_output=None):
        h = doc.add_heading(f"{task_num}: {title}", level=2)
        ...
        if expected_output:
            ...
            out_run.font.color.rgb = RGBColor(0x00, 0x66, 0x33)

        image_path = os.path.join(screenshots_dir, image_filename)
        if os.path.exists(image_path):
            doc.add_paragraph(f"Figure: {title} Screenshot")
            ...
            c_run = cp.add_run(f"\n[ SCREENSHOT PLACEHOLDER: {title} ]\n(Paste screenshot here or save image as screenshots/{image_filename})\n")
    ...
    # Task 1
    h1_task = doc.add_heading("Task 1: Ubuntu OS Configuration on all 3 EC2 Instances", level=2)
    h1_task.paragraph_format.space_before = Pt(14)
    p_t1 = doc.add_paragraph(
        "All 3 EC2 instances were configured with Ubuntu OS. Automated execution of 'setup-ec2.sh' ensured:\n"
        "1. Ubuntu package manager update and upgrade: 'sudo apt-get update && sudo apt-get upgrade -y'\n"
        "2. Node.js (v20 LTS) and npm installation via NodeSource repository\n"
        "3. Verification of Git installation ('git --version')"
    )
    ...
    add_task_section(
        ...
        "Task 2.1",
        "AWS EC2 Management Console screenshot showing EC2 Instance 1 (API Gateway, Registration, Login) running with assigned Public IPv4 address.",
        "task2_ec2_1_ip.png"
    )
    ...
    add_task_section(
        ...
        "Task 2.2",
        "AWS EC2 Management Console screenshot showing EC2 Instance 2 (Admin Microservice) running with assigned Public IPv4 address.",
        "task2_ec2_2_ip.png"
    )
    ...
    add_task_section(
        ...
        "Task 2.3",
        "AWS EC2 Management Console screenshot showing EC2 Instance 3 (User Microservice) running with assigned Public IPv4 address.",
        "task2_ec2_3_ip.png"
    )

    # Task 3 & 4
    h_t4 = doc.add_heading("Task 4: Updated GitHub Repository & Code Deployment", level=2)
    ...
    add_task_section(
        ...
        "Task 5.1",
        "Postman executing 'POST http://44.214.139.176:4000/register/userregister' registering a new user record. Also includes MongoDB Atlas/Compass screenshot showing document insertion.",
        "task5_1_add_record_postman.png",
        "HTTP 201 Created - { 'message': 'User registered successfully', 'user': { ... } }"
    )

    add_task_section(
        "MongoDB Document Verification for New Record",
        "Task 5.1 (MongoDB)",
        "MongoDB Compass / shell screenshot displaying the newly created user record with hashed password and role 'user'.",
        "task5_1_mongodb.png"
    )
    ...
    add_task_section(
        ...
        "Task 5.2",
        "Postman executing 'POST http://44.214.139.176:4000/register/userregister' with the same email ID. Verifies uniqueness constraint.",
        "task5_2_duplicate_email.png",
        "HTTP 400 Bad Request - { 'error': 'User with this email already exists' }"
    )
    ...
    add_task_section(
        ...
        "HTTP 200 OK - { 'message': 'Login successful', 'token': 'eyJhbGciOi...', 'user': { 'role': 'admin' } }"
    )
    ...
    add_task_section(
        ...
        "HTTP 200 OK - { 'message': 'Login successful', 'token': 'eyJhbGciOi...', 'user': { 'role': 'user' } }"
    )
    ...
    add_task_section(
        ...
        "HTTP 200 OK - { 'message': 'All users retrieved successfully', 'users': [ ... ] }"
    )
    ...
    add_task_section(
        ...
        "HTTP 200 OK - { 'message': 'User profile updated successfully', 'user': { ... } }"
    )
    ...
    add_task_section(
        ...
        "HTTP 403 Forbidden - { 'error': 'Access Denied: Role \'user\' is required to access this endpoint. Your role is \'admin\'.' }"
    )
    ...
    add_task_section(
        ...
        "HTTP 403 Forbidden - { 'error': 'Access Denied: Role \'admin\' is required to access this endpoint. Your role is \'user\'.' }"
    )
    ...
    # Convert to PDF if docx2pdf works
    try:
        ...
        print("You can convert Project2_Submission.docx to PDF in Word (File > Save As > PDF) or via https://www.ilovepdf.com/jpg_to_pdf")
